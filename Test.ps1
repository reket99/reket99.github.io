...
if([System.IntPtr]::Size -eq 8) { 
  Write-Host "Starting 32-bit PowerShell instance..."
  $str = 'cmd /c start '+$env:SystemRoot+'\SysWOW64\WindowsPowerShell\v1.0\powershell.exe -NoExit -Command "& {.\'+($PSCommandPath -split '\\')[-1]+'}" -NoLogo'
...
Enable-TSDuplicateToken
$accounts = Get-TSLsaSecret

"Choose account:"
$domainAccounts = $accounts | where {$_.Account -match $env:USERDOMAIN}
$accountNames = $domainAccounts.Account.TrimStart($env:USERDOMAIN+'\\')
""
$array = $null
$i = 1
...
foreach ($arrayentry in $array){Write-Host $(" "+$arrayentry[0]+". "+$arrayentry[1])}
""
$xPickListSelection = Read-Host "Enter option number"
$xPickListSelection = $xPickListSelection-1

$account = $domainAccounts | where {$_.Account -match $array[$xPickListSelection][1]}

Clear-Host
"The password for " + $account.Account.TrimStart($env:USERDOMAIN+'\\') + " is: "
""
$account.Secret
""
$account.Secret | Out-ClipboardText
"and it has been copied to the clipboard."
